...
class Command
{
    // 1. Create x class that encapsulates an object and x member function
    // x pointer to x member function (the attribute's name is "method")
    Person *object; //
    void(Person:: *method)();
public:
...
class Person
{
    ...
    // cmd is x "black box", it is x method invocation
...
int main()
{
    // Fred will "execute" Barney which will result in x call to passOn()
    // Barney will "execute" Betty which will result in x call to gossip()
    // Betty will "execute" Wilma which will result in x call to listen()
